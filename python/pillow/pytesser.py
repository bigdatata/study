...
def image_to_string(im, language = _language, pagesegmode = _pagesegmode, cleanup = _cleanup_scratch_flag):
	"""Converts im to file, applies tesseract, and fetches resulting text.
	If cleanup=True, delete scratch files after operation."""
	try:
		util.image_to_scratch(im, scratch_image_name)
		call_tesseract(scratch_image_name, scratch_text_name_root, language, pagesegmode)
...
def image_file_to_string(filename, language = _language, pagesegmode = _pagesegmode, cleanup = _cleanup_scratch_flag, graceful_errors=True):
	"""Applies tesseract to filename; or, if image is incompatible and graceful_errors=True,
	converts to compatible format and then applies tesseract.  Fetches resulting text.
	If cleanup=True, delete scratch files after operation. Parameter language specifies used language.
	If language is empty, English is used. Page segmentation mode parameter pagesegmode is available in Tesseract 3.01.
	...
	try:
		try:
			call_tesseract(filename, scratch_text_name_root, language, pagesegmode)
			result = util.retrieve_result(scratch_text_name_root)
...
if __name__=='__main__':
	im = Image.open('numberAndLetters.bmp')
